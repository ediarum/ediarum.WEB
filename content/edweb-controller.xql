...
) as node()* 
{
    ...
    let $matches := for $part at $pos in $path-regex-parts
         let $path-tail := if (fn:count($path-regex-parts) eq 1) then ($path) else (replace($path, "^"||string-join(for $p in subsequence($path-regex-parts, 1, $pos -1) return $p(1)), ""))let $match := functx:get-matches($path-tail, $part(1))[1]
